...
while ($true) {
  ...
  if ($PSCmdlet.ParameterSetName -eq 'Org') {
    if ($Visibility -ne 'all') { $url += "&type=$Visibility" }
  }
  $resp = Get-GHPage $url
...
$subject = if ($PSCmdlet.ParameterSetName -eq 'Org') { $Org } else { $User }
if ($all.Count -eq 0) { Write-Warn ("No repositories returned by GitHub for {0}." -f $subject); return }
