...
function Write-StepWarning { param([string]$Message); Write-Host "  ??  $Message" -ForegroundColor Yellow }
function Write-StepInfo { param([string]$Message); Write-Host "  ??  $Message" -ForegroundColor Cyan }
...
switch ($Action) {
    ...
    'security' {
        Write-Host "`n???  Running security scan & attestation..." -ForegroundColor Yellow
        New-NodeIdentity
        Test-Tailscale
        Invoke-IpAudit
        Invoke-FleetAudit
