...
# Recorder validation gate -----------------------------------------------
try {
    Write-Log -Message 'Invoking recorder validation harness...' 
    $recorderHarness = Join-Path $WorkspacePath 'scripts/Recorder-Validation-Harness.ps1'
    if(Test-Path $recorderHarness){
        pwsh -File $recorderHarness -EventsRoot 'data/recorder/events' -PythonExe 'python'
        if($LASTEXITCODE -ne 0){ throw "Recorder validation failed (exit $LASTEXITCODE)" }
        Write-Log -Message 'Recorder events validated successfully.'
    } else {
        Write-Log -Level WARN -Message 'Recorder harness script not found; skipping validation gate.'
    }
} catch {
    Write-Log -Level ERROR -Message $_.Exception.Message
