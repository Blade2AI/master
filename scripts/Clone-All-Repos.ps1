...
# Ensure Git will trust repos anywhere (helps with UNC/network locations)
try { & git config --global --add safe.directory '*' 2>$null } catch {}

# Prepare auth args if GITHUB_TOKEN is present (helps clone/pull private repos over HTTPS)
$gitAuthArgs = @()
if ($env:GITHUB_TOKEN) {
    $gitAuthArgs = @('-c', "http.extraHeader=Authorization: Bearer $($env:GITHUB_TOKEN)")
}
    ...
    try {
        ...
        if ($isGit) {
            if ($PullIfExists) {
                ...
                Log "FETCH $dest"
                & git @gitAuthArgs -C $dest fetch --all --tags 2>&1 | Tee-Object -FilePath $logPath -Append | Out-Null
                ...
                & git @gitAuthArgs -C $dest pull 2>&1 | Tee-Object -FilePath $logPath -Append | Out-Null
        ...
        elseif ($destExists -and -not $isGit) {
            if ($ForceReclone) {
                ...
                & git @gitAuthArgs clone $e.url $dest 2>&1 | Tee-Object -FilePath $logPath -Append | Out-Null
