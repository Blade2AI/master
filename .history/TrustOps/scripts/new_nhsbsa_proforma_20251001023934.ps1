...
function JoinLines($text) {
  ...
  ($text -split '[\r\n,]+' | ForEach-Object { $_.Trim() } | Where-Object { $_ } | ForEach-Object { "- $_" }) -join "`n"
}
$datasetLines = if ($Datasets) { JoinLines $Datasets } else { '' }
$fieldLines = if ($FieldList) {
  $i = 0
  ($FieldList -split '[\r\n,]+' | ForEach-Object { $_.Trim() } | Where-Object { $_ } | ForEach-Object { $script:i++; "$script:i) $_" }) -join "`n"
} else { '' }
...
# Literal placeholder replacement (no regex)
foreach ($k in $map.Keys) { $content = $content.Replace($k, [string]$map[$k]) }

# Insert datasets and fields blocks (will be empty if not provided)
$content = $content.Replace('{{DATASETS_BLOCK}}', $datasetLines)
$content = $content.Replace('{{FIELDS_BLOCK}}', $fieldLines)
