...
function New-BoardroomSelfSignedCert([string]$cn) {
...
function Bind-HttpSysCert([int]$port, [string]$thumbprint) {
...
$cert = New-BoardroomSelfSignedCert -cn $CertCN
Bind-HttpSysCert -port 443 -thumbprint $cert.Thumbprint
Bind-HttpSysCert -port 8444 -thumbprint $cert.Thumbprint
