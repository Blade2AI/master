...
while ($true) {
  ...
  try {
    ...
    if ($context.Request.IsSecureConnection -and $context.Request.ClientCertificate) {
      $script:ClientCertThumbprint = $context.Request.ClientCertificate.GetCertHashString()
