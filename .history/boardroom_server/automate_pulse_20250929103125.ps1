...
function Estimate-Impact([string]$text) { if ($text -match '(?i)leak|energy') { 15000 } else { 5000 } }

function Transmute-Idea([string]$text) {
  if ($text -match '(?i)co-?op\s*(?<type>\w+)?') {
    $t = $Matches['type']; if (-not $t) { $t = 'Energy' }
    return "New-CoOp -Type $t"
  }
  return "Write-Output \"IDEA:$text\""
}
...
function Update-Status([int]$ideasProcessed) {
  $cfg = Read-Config
  $now = Get-Date
  $status = @{
    pulse = 'active'
  ...
  ($status | ConvertTo-Json -Depth 6) | Set-Content -LiteralPath (Read-Config).StatusFile -Encoding UTF8
}
...
while ($true) {
  ...
  foreach ($idea in $ideas) {
    try {
      ...
      $impact = if ($idea.PSObject.Properties['ImpactEstimate']) { [int]$idea.ImpactEstimate } else { Estimate-Impact $text }
  ...
  Update-Status -ideasProcessed $processed
