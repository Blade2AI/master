...
Write-Host 'Registering HTTPS dashboard service (on login) ...' -ForegroundColor Yellow
$taskName2 = 'BoardroomHttpsServer'
$action2 = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$PSScriptRoot\\Start-BoardroomHttpsServer.ps1`""
$trigger2 = New-ScheduledTaskTrigger -AtLogOn
Register-ScheduledTask -TaskName $taskName2 -Action $action2 -Trigger $trigger2 -RunLevel Highest -Force | Out-Null

Write-Host 'Registering Idea Injection server (on login) ...' -ForegroundColor Yellow
$taskName3 = 'BoardroomInjectionServer'
$action3 = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$PSScriptRoot\\Start-BoardroomInjectionServer.ps1`""
$trigger3 = New-ScheduledTaskTrigger -AtLogOn
Register-ScheduledTask -TaskName $taskName3 -Action $action3 -Trigger $trigger3 -RunLevel Highest -Force | Out-Null

Write-Host 'Registering Automation Daemon (on login) ...' -ForegroundColor Yellow
$taskName4 = 'BoardroomAutomationDaemon'
$action4 = New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$PSScriptRoot\\automate_pulse.ps1`""
$trigger4 = New-ScheduledTaskTrigger -AtLogOn
Register-ScheduledTask -TaskName $taskName4 -Action $action4 -Trigger $trigger4 -RunLevel Highest -Force | Out-Null
