...
BatchMerkleProof build_batch_merkle_proof(const std::vector<std::string>& paths,
    ...
    // Build levels bottom-up (vector of vectors of hashes)
    ...
    // Build per-leaf paths
    for (size_t idx : targetIndices) {
        if (idx >= n) continue;
        BatchLeafProof lp; lp.leafIndex = idx; lp.leafHashHex = to_hex(leaves[idx]);
        size_t pos = idx; // position in current level
        for (size_t lvl = 0; lvl < levels.size()-1; ++lvl) {
            const auto &cur = levels[lvl];
            size_t sibling = (pos % 2 == 0) ? pos + 1 : pos - 1;
            if (sibling >= cur.size()) sibling = pos; // mirrored if odd count
            lp.siblingHex.push_back(to_hex(cur[sibling]));
            lp.leafIsLeft.push_back(pos % 2 == 0); // record orientation BEFORE moving to parent
            pos = pos / 2; // move up to parent index in next level
        }
        proof.leaves.push_back(lp);
    }
    return proof;
}

bool verify_batch_merkle_proof(const BatchMerkleProof& proof) {
    if (proof.leaves.empty() || proof.rootHex.empty()) return false;
    for (const auto &lp : proof.leaves) {
        std::string runningHex = lp.leafHashHex;
        if (lp.siblingHex.size() != lp.leafIsLeft.size()) return false;
        for (size_t i = 0; i < lp.siblingHex.size(); ++i) {
            // decode hex to bytes
            auto hexToBytes = [](const std::string &hex){
                std::vector<unsigned char> out; out.reserve(hex.size()/2);
                for (size_t j=0;j<hex.size(); j+=2){ out.push_back(static_cast<unsigned char>(std::stoul(hex.substr(j,2), nullptr,16))); }
                return out;
            };
            auto lhs = hexToBytes(runningHex);
            auto rhs = hexToBytes(lp.siblingHex[i]);
            std::string concat;
            concat.reserve(lhs.size()+rhs.size());
            if (lp.leafIsLeft[i]) {
                concat.append(reinterpret_cast<const char*>(lhs.data()), lhs.size());
                concat.append(reinterpret_cast<const char*>(rhs.data()), rhs.size());
            } else {
                concat.append(reinterpret_cast<const char*>(rhs.data()), rhs.size());
                concat.append(reinterpret_cast<const char*>(lhs.data()), lhs.size());
            }
            runningHex = to_hex(compute_sha256(concat));
        }
        if (runningHex != proof.rootHex) return false; // leaf path invalid
    }
    return true; // all leaf paths reached root
}
